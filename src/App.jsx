...
export default function App() {
  ...
  const [sheet, setSheet] = useState(null); // { itemId, view: 'menu'|'add'|'view' }
            ...
            <div key={task.id} style={S.taskCard}
              ...
              <div style={S.taskRow}>
                <div draggable style={S.grip}
                  onDragStart={e => onDragStart(e, "task", null, null, ti)}>⠿</div>
                ...
                <div style={{ ...S.checkCircle, ...(task.done ? S.checkDone : {}) }}
                  onClick={() => toggleDone("task", task.id)} />
                <div style={S.taskInfo} onClick={() => toggleTask(task.id)}>
                  ...
                  {hasSubs && (
                    <div style={S.progressBg}>
                      <div style={{ ...S.progressFill, width: `${Math.round(doneSubs / task.subs.length * 100)}%` }} />
                    </div>
                  )}
                </div>
                    ...
                    return (
                      <div key={sub.id}
                        ...
                        <div style={S.subRow}>
                          ...
                          <div style={{ ...S.subCheck, ...(sub.done ? S.checkDone : {}) }}
                            onClick={() => toggleDone("sub", task.id, sub.id)} />
                          ...
                          <div style={S.ssubList}>
                            {sub.ssubs.map((ss, ssi) => (
                              <div key={ss.id} style={S.ssubRow}
                                ...
                                <div style={{ ...S.ssubCheck, ...(ss.done ? S.checkDone : {}) }}
                                  onClick={() => toggleDone("ssub", task.id, sub.id, ss.id)} />
            ...
            <div style={{ padding: "0 16px 28px", overflowY: "auto" }}>
              ...
              {sheet.view === "menu" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingTop: 8 }}>
                  <SheetOption icon="➕" title="Add" sub="Add a picture, URL link or note" color="#E6F1FB" textColor="#185FA5" onClick={() => setSheet({ ...sheet, view: "add" })} />
                  <SheetOption icon="👁️" title="View all" sub={attCount > 0 ? `${attCount} item(s) attached` : "Nothing added yet"} color="#FAEEDA" textColor="#BA7517" onClick={() => setSheet({ ...sheet, view: "view" })} />
                </div>
              )}
              ...
              {sheet.view === "add" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14, paddingTop: 8 }}>
                  ...
                  <div>
                    ...
                    <input style={S.formInput} value={urlInput} onChange={e => setUrlInput(e.target.value)} placeholder="https://..." />
                  </div>
                  <div>
                    <div style={S.formLabel}>📝 Notes</div>
                    <textarea style={{ ...S.formInput, height: 80, resize: "none" }} value={noteInput} onChange={e => setNoteInput(e.target.value)} placeholder="Write your note here..." />
                  </div>
...
function SheetOption({ icon, title, sub, color, textColor, onClick }) {
  return (
...
const styles = {
  ...
  taskCard: { background: "#fff", border: "0.5px solid #eee", borderRadius: 12, overflow: "visible" },
  ...
  checkCircle: { width: 18, height: 18, borderRadius: "50%", border: "1.5px solid #ddd", flexShrink: 0, cursor: "pointer", marginLeft: 8, display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  subCheck: { width: 14, height: 14, borderRadius: "50%", border: "1.5px solid #ddd", flexShrink: 0, cursor: "pointer", marginLeft: 8, display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  ssubRow: { display: "flex", alignItems: "center", padding: "5px 8px 5px 0", background: "#fff", border: "0.5px solid #eee", borderRadius: 8 },
  ...
  ssubCheck: { width: 12, height: 12, borderRadius: "50%", border: "1.5px solid #ddd", flexShrink: 0, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  delBtn: { position: "absolute", top: 6, right: 6, width: 18, height: 18, borderRadius: "50%", background: "#FCEBEB", border: "none", cursor: "pointer", fontSize: 10, color: "#A32D2D", display: "flex", alignItems: "center", justifyContent: "center" },
};